...
A behavior earns its place when the base has a habit worth overriding. These
three show that clearly. Others do not: a medical behavior adds little to a
model this size, which already knows the material, and a politeness behavior
adds almost nothing to a model that instruction tuning has already made polite.

**Verbosity.** Asked for a function, the base writes a docstring, type hints, a
note about duplicate keys, an example and its printed output. With the code
behavior switched on it writes the function.

**The answer itself.** On the train question the base and the bank disagree, and
the bank is right. Two trains, 30 km/h of difference, 45 km of head start: 1.5
hours from 14:50, so 16:20.

**Length, and how sharply it turns.** The summary behavior is pinned here so the
router cannot dilute it. Between 0.75 and 1.0 the answer collapses by an order of
magnitude, which is worth seeing: the dial is not always smooth.
""")

code('''
import textwrap

def gen(prompt, gammas=None, max_new=420, base=False, temperature=0.8, pin=None):
    """gammas is {name: strength}. Anything omitted is silenced.

    pin overrides the router with fixed weights, for when you want one behavior
    applied regardless of what the router makes of the prompt.
    """
    for n in names:
    ...
    ctx = bank.disabled() if base else (bank.pin(pin) if pin else S._null())
...
def show(title, text):
    print()
    print(f"{title}   [{ntok(text)} tokens]")
    print(chr(9472) * 80)
    for para in text.split(chr(10)):
        print(textwrap.fill(para, 80) if para.strip() else "")


all_on = {n: 1.0 for n in names}

# 1. code. The base explains at length; the behavior answers and stops.
Q = "Write a Python function that reverses a dictionary."
show("everything on, code muted", gen(Q, {**all_on, "code": 0.0}))
show("everything on", gen(Q, all_on))

# 2. math. Here the difference is the answer, not the presentation.
Q = ("A train leaves at 14:20 travelling at 90 km/h. Another leaves the same "
     "station at 14:50 at 120 km/h. When does the second catch the first?")
show("frozen base", gen(Q, base=True))
show("everything on", gen(Q, all_on))

# 3. summary, pinned so the router cannot dilute it. The base answer runs to
# several hundred tokens and is not shown; only its length is, for comparison.
Q = "What is the difference between TCP and UDP?"
print(f"the frozen base answers this in {ntok(gen(Q, base=True))} tokens")
show("summary pinned at 0.75", gen(Q, pin={"summary": 0.75}))
show("summary pinned at 1.0", gen(Q, pin={"summary": 1.0}))
''')
...
code('''
ROUTE_Q = "Write a Python function that reverses a dictionary."
ids = tok(S._chatml(tok, ROUTE_Q), return_tensors="pt",
