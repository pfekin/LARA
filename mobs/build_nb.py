...
md("""
## One question, five behaviors

The same question put to each behavior in turn. Every one of them gets the
arithmetic right, so what is left is manner, and that is the part worth reading:

- **code** answers in Python and skips the prose
- **math** derives the number first, then writes the function
- **medical** gives the answer in a line and works an example through to an
  unsafe case
- **polite** numbers the steps and explains each parameter
- **summary** writes a function that documents its own clinical caveat

Each is shown at 1.0, the strength it was trained at. The faders above go
further. How far is useful depends on the bank, so it is worth finding by hand
...
def gen(prompt, gammas=None, max_new=420, base=False, temperature=0.8):
...
for n in names:
    show(f"{n} at 1.0", gen(PROMPT, {n: 1.0}))
''')
